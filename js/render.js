...
const Render = {
  ...
  drawEnemy(e){
    ...
    if(e.atkAnim>0){
      ...
      ctx.beginPath();
      ctx.arc(s.x, bodyY-hgt*0.5, r*2.2, a-0.6,a+0.6);
      ctx.stroke();
    }
    // 血条
    if(e.hp<e.maxHp){
      ...
      ctx.fillRect(s.x-w/2, bodyY-hgt-r*1.1, w,4);
      ctx.fillStyle = pct>0.5?'#5cb85c':(pct>0.25?'#f0ad4e':'#d33');
      ctx.fillRect(s.x-w/2, bodyY-hgt-r*1.1, w*pct,4);
    }
    // 精英词缀标记
    if(e.eliteAffix){
      ctx.font='12px serif'; ctx.textAlign='center';
      ctx.fillStyle='#ffd700';
      ctx.fillText(e.eliteAffix.name, s.x, bodyY-hgt-r*1.3-6);
    }
  },
  ...
  drawBeast(b){
    ...
    const bodyY=s.y-bob;
    const evScale = 1+b.evolution*0.5;
    // 幻兽：紫色幽焰兽
    ...
    if(b.level>1){
      ...
      ctx.fillText('Lv'+b.level, s.x, bodyY-40*evScale-6);
    }
  },
  ...
  drawPlayer(p){
    ...
    if(p.attackAnim>0){
      ...
      ctx.beginPath();
      ctx.moveTo(s.x+Math.cos(a)*16, bodyY-30+Math.sin(a)*16);
      ctx.lineTo(s.x+Math.cos(a)*40, bodyY-30+Math.sin(a)*40);
      ctx.stroke();
      ctx.fillStyle='rgba(255,255,255,0.4)';
      ctx.beginPath(); ctx.arc(s.x+Math.cos(a)*40, bodyY-30+Math.sin(a)*40, 8,0,7); ctx.fill();
    } else {
      ctx.strokeStyle='#a0a0a0'; ctx.lineWidth=3;
      ctx.beginPath();
      ctx.moveTo(s.x+12, bodyY-20);
      ctx.lineTo(s.x+22, bodyY-36);
      ctx.stroke();
    }
    // 狂暴状态光效
    if(p.furyT>0){
  ...
  drawAmbient(){
    ...
    // 屏幕震动
    if(fx.shake>0){
      fx.shake = Math.max(0, fx.shake-0.02);
      ctx.save();
