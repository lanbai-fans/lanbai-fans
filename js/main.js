...
const game = {
  ...
  killTarget:0, doorActive:false, doorPos:null, doorReadyT:0, lastKillPos:null,
  runStart:0, time:0, last:0,
  meta:{ souls:0, atkUp:0, hpUp:0, lootUp:0, upgradeCount:0, nextCost:10 },
  maxFloorReached:1,
  touchMode:false,          // 触屏模式（技能自动瞄准）
  tutorialShown:false,      // 新手教程只弹一次
};
...
function setupRun(){
  ...
  // recomputeStats 已包含等级成长与元强化（跨局变强）加成
...
function loadFloor(){
  ...
  game.doorReadyT=0; game.lastKillPos=null;
  // 波次预算（Boss层清空后触发Boss）
...
function updateSpawning(dt){
  // Boss 层判定：每 bossEvery 层一个魔王（5/10/15/20/25/30…）
  const isBossFloor = game.floor % CFG.dungeon.bossEvery === 0;
  // 本层清空后
  if(game.enemies.length===0 && game.spawnQueue.length===0 && game.spawnBudget<=0){
    // Boss 层：清空杂兵后生成 Boss（尚未生成且未击败时）
    if(isBossFloor && !game.bossSpawned && !game.bossDefeated){
    ...
    // 开门：普通层清空即开门；Boss 层在 Boss 被击败后开门（BossDefeated 不再阻断开门流程）
    // 门生成在最后击杀的怪物脚下（lastKillPos），Boss 层则开在 Boss 脚下
    if(!game.doorActive && (game.bossDefeated || !isBossFloor)){
      const lk = game.lastKillPos;
      openDoor(lk? lk.x : undefined, lk? lk.y : undefined);
    }
    return;
  }
  game.spawnTimer -= dt;
...
/* 开门：在最后击杀的怪物脚下生成（用户需求），无记录时回退随机位置 */
function openDoor(x, y){
  ...
  if(typeof x==='number' && typeof y==='number'){
    ...
    pos = findSpawnNear(game.player.x+Math.cos(ang)*edgeR, game.player.y+Math.sin(ang)*edgeR, 40, 10);
  }
  game.doorPos = pos;
  game.doorReadyT = 0.8;   // 门出现 0.8s 后才可传送，避免瞬间吸人
  showMsg('裂界之门开启！前往深处');
}
...
function useSkill(id){
  ...
  switch(id){
    case 'fire': {
      // 瞄准：鼠标位置；触屏/无鼠标时自动瞄准最近敌人（移动端适配）
      let aim = game.mouse.world;
      if(game.touchMode || !aim){
        ...
        aim = best? {x:best.x, y:best.y} : {x:p.x+Math.cos(p.facing)*200, y:p.y+Math.sin(p.facing)*200};
      }
...
function updatePlayer(dt){
  ...
  if(p.dashT>0){
    p.dashT-=dt;
    // 冲刺速度 520/s × 0.22s ≈ dashDistance(110)
...
function updateFx(dt){
  ...
  // 震屏按时间衰减（原在渲染层按帧衰减，帧率不稳时抖动时长失真）
  if(fx.shake>0) fx.shake = Math.max(0, fx.shake - dt*1.6);
}
...
function gameOver(win){
  ...
  while(game.meta.souls >= game.meta.nextCost){
    ...
    game.meta.nextCost = 10 + game.meta.upgradeCount*3;   // 先升级再算下一档成本（修复首两档成本重复为 10 的时序 bug）
    spent++;
  }
...
/* 进入下一层（Boss 层/普通层通用；30 层通关后进入无尽深渊） */
...
game.startRun = function(){
  ...
  setupRun();
  loadFloor();
  game.paused=false;                 // 修复：重开时 paused 残留导致画面冻结
  UI.updateHUD();
  if(!game.tutorialShown){           // 教程只弹一次（原来每次重开都弹）
...
function doDodge(){
  const p = game.player;
  if(!p || game.screen!=='playing' || game.paused) return;
...
function bindInput(){
  ...
  /* 鼠标 / 触摸共用：按下（点地面移动 / 点怪锁定攻击） */
  function pointerDown(sx, sy){
    game.mouse.x=sx; game.mouse.y=sy;
    const w=toWorld(sx,sy,Render.cam);
    game.mouse.world=w; game.mouse.down=true;
    let t=null,bd=44;
    ...
    game.mouse.target = t || null;   // 点空地清除锁定，点怪锁定
  }
  function pointerMove(sx, sy){
    game.mouse.x=sx; game.mouse.y=sy;
    game.mouse.world = toWorld(sx,sy,Render.cam);
  }
  function pointerUp(){ game.mouse.down=false; }
  ...
  window.addEventListener('blur', ()=>{ game.mouse.down=false; game.keys={}; });
}
...
function frame(t){
  ...
  if(game.screen==='playing' && !game.paused){
    ...
    updatePickup(dt);
    updateFx(dt);
    // 门推进：靠近门（门冷却结束）→ 弹出肉鸽卡牌二选一 → 进入下一层
    if(game.doorReadyT>0) game.doorReadyT-=dt;
    if(game.doorActive && game.doorPos && game.doorReadyT<=0){
...
/* ?auto=1 演示模式：自动开局+自动战斗+无敌（用于无头验证/快速预览） */
const AUTO = /[?&]auto=1/.test(location.search);
if(AUTO){
  window.addEventListener('load', ()=>{
    game.tutorialShown = true;
    game.startRun();
    let autoT = 0, lastAtk = 0;
    const origUpdate = game.update || function(){};
    game.autoAtk = true;
    ...
    const autoTimer = setInterval(()=>{
      ...
      if(now - lastAtk > 450){
        lastAtk = now;
        const target = game.enemies.reduce((b,e)=>!e.dead && (!b || dist(p,e)<dist(p,b)) ? e : b, null);
        if(target){
          game.mouse.target = target;
          playerMelee(p, game.enemies);
        } else {
          const a = Math.random()*Math.PI*2;
          game.mouse.world = { x:p.x+Math.cos(a)*200, y:p.y+Math.sin(a)*200 };
        }
      }
    }, 100);
  });
