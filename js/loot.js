...
function rollDrops(floor, enemyType){
  ...
  const L = floor;
  // 卡牌掉落加成（贪婪之印等）
  const lootBonus = 1 + ((game.player && game.player.card && game.player.card.loot)||0)/100;
  if(enemyType==='boss'){
    ...
    drops.push(rollItem(L, forced));
    if(chance(0.35*lootBonus)) drops.push(rollItem(L));
    souls = CFG.enemies.elite.soul;
  } else {
    if(chance(Math.min(0.9, cfg.baseDropChance*lootBonus))) drops.push(rollItem(L));
    // 普通怪保底 1 魂晶，确保幻兽可持续喂食成长
    souls = chance(Math.min(0.9, cfg.soulChance*lootBonus)) ? CFG.enemies.melee.soul + Math.floor(L/10) : 1;
  }
  return { drops, souls };
}
