...
function createPlayer(x,y){
  return {
    ...
    armor:0, regen: CFG.player.regen,
    // 肉鸽卡牌强化（过层二选一累计）
    card: {
      atk:0, dmg:0, as:0, range:0, hp:0, regen:0, ms:0, crit:0, critD:0,
      loot:0, beastAtk:0, beastHp:0, souls:0,
      skillLv:{ strike:0, fire:0, nova:0, fury:0 },   // 相对1级的额外技能等级
    },
...
function recomputeStats(p){
  // 基础值 + 等级成长 + 元强化（跨局）+ 肉鸽卡牌（本局）——统一在此计算，避免被重算清零
  const c = p.card || { atk:0,dmg:0,as:0,range:0,hp:0,regen:0,ms:0,crit:0,critD:0,loot:0,beastAtk:0,beastHp:0,souls:0,skillLv:{strike:0,fire:0,nova:0,fury:0} };
  p.atk = CFG.player.baseAtk + c.atk + game.meta.atkUp*3;
  p.speed = CFG.player.baseSpeed * (1 + c.as/100);
  p.critChance = CFG.player.critChance + c.crit;
  p.critMult = CFG.player.critMult + c.critD;
  p.moveSpeed = CFG.player.moveSpeed * (1 + c.ms/100);
  p.maxHp = CFG.player.maxHp + (p.level-1)*10 + c.hp + game.meta.hpUp*15;
  p.maxMp = CFG.player.maxMp + (p.level-1)*6;
  p.regen = CFG.player.regen + c.regen;
  p.armor = 0;
...
function recomputeBeast(b){
  // 幻兽成长 + 卡牌幻兽强化（本局）
  const c = (game.player && game.player.card) || { beastAtk:0, beastHp:0 };
  b.atk = (CFG.beast.baseAtk + (b.level-1)*CFG.beast.atkPerLevel) * (1 + c.beastAtk/100);
  b.maxHp = (CFG.beast.maxHp + (b.level-1)*CFG.beast.hpPerLevel) * (1 + c.beastHp/100);
